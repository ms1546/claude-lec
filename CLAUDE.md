...
学習体験の継続性を保つため、`.claude/session/`ディレクトリでセッション情報を管理します：
...
### LEARNING_STATUS.mdの活用

プロジェクトルートの`LEARNING_STATUS.md`は、人間が読める形式で全体の進捗を記録します。このファイルは：
- エンジニアが進捗を確認しやすい
- 新しいセッションで前回の状態を素早く把握
- 学習の振り返りに活用

### セッション開始時の確認

新しいセッションを開始する際は、必ず以下を確認してください：

1. `.claude/session/current-session.json`から前回の状態を読み取る
2. `LEARNING_STATUS.md`で全体の進捗を確認
3. `next-steps.md`から次のアクションを把握
4. エンジニアに継続か新規開始かを確認
